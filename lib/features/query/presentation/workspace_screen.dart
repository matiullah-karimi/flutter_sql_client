...
class _TableStructureDialogState extends ConsumerState<_TableStructureDialog> {
        ...
        query =
          ...
          SELECT 
            ...
            (SELECT COUNT(*) FROM information_schema.table_constraints tc
             ...
             WHERE tc.table_name = c.table_name 
               AND kcu.column_name = c.column_name 
               AND tc.constraint_type = 'UNIQUE') > 0 as is_unique
          FROM information_schema.columns c
          WHERE c.table_name = '${widget.tableName}'
  ...
  Future<void> _addColumn() async {
    ...
    try {
      ...
      if (_isUnsigned && adapter is MysqlAdapter) {
        type += ' UNSIGNED';
      }

      String constraints = '';
      if (_isPrimaryKey) constraints += ' PRIMARY KEY';
      if (_isUnique) constraints += ' UNIQUE';
      ...
      final alterQuery =
        ...
        ADD COLUMN ${_nameController.text} $type
            ...
            Wrap(
              ...
              children: [
                FilterChip(
                  ...
                  selected: _isPrimaryKey,
                  onSelected: (v) => setState(() => _isPrimaryKey = v),
                ),
                FilterChip(
                  label: const Text('Unique'),
                  selected: _isUnique,
                  onSelected: (v) => setState(() => _isUnique = v),
                ),
                FilterChip(
                  label: const Text('Auto Increment'),
                  selected: _isAutoIncrement,
                  onSelected: (v) => setState(() => _isAutoIncrement = v),
                ),
                FilterChip(
                  label: const Text('Unsigned'),
                  selected: _isUnsigned,
                  onSelected: (v) => setState(() => _isUnsigned = v),
                ),
              ],
            ),
            const SizedBox(height: 16),
  ...
  Widget _buildColumnItem(int index, Map<String, dynamic> column) {
    ...
    if (column.containsKey('is_primary')) {
      // Postgres
      isPrimary = column['is_primary'] == true || column['is_primary'] == 'YES';
    } else if (column.containsKey('column_key')) {
      // MySQL
      isPrimary = column['column_key'] == 'PRI';
    }

    if (column.containsKey('is_unique')) {
      // Postgres
      isUnique = column['is_unique'] == true || column['is_unique'] == 'YES';
    } else if (column.containsKey('column_key')) {
      // MySQL
    ...
    if (column.containsKey('extra')) {
      ...
      isAutoIncrement =
        ...
        columnDefault.toString().contains('nextval')) {
      // Postgres
    ...
    if (column.containsKey('column_type')) {
      // MySQL
      isUnsigned =
          column['column_type']?.toString().contains('unsigned') ?? false;
    }
      ...
      child: ListTile(
        ...
        subtitle: Column(
          ...
          children: [
            ...
            if (isPrimary || isUnique || isAutoIncrement || isUnsigned)
              Padding(
                padding: const EdgeInsets.only(top: 4),
                child: Wrap(
                  spacing: 4,
                  children: [
                    if (isPrimary) _buildBadge('PK', Colors.amber),
                    if (isUnique) _buildBadge('UK', Colors.blue),
                    if (isAutoIncrement) _buildBadge('AI', Colors.green),
                    if (isUnsigned) _buildBadge('UN', Colors.purple),
                  ],
                ),
              ),
          ],
  ...
  Future<void> _showEditColumnDialog(
    ...
    final nameController = TextEditingController(text: columnName);
    final typeController = TextEditingController(text: currentType);
    final defaultController = TextEditingController(text: currentDefault ?? '');
    ...
    await showDialog(
      ...
      builder: (context) => StatefulBuilder(
        builder: (context, setState) => AlertDialog(
          title: Text('Edit Column: $columnName'),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              TextField(
              ...
              CheckboxListTile(
                title: const Text('Nullable'),
                value: nullable,
                onChanged: (v) => setState(() => nullable = v ?? true),
              ),
            ],
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                try {
                  ...
                  // Rename
                  if (nameController.text != columnName) {
                    await adapter.query(
                      'ALTER TABLE ${widget.tableName} RENAME COLUMN "$columnName" TO "${nameController.text}"',
                    );
                  }
                  ...
                  // Type
                  if (typeController.text != currentType) {
                    await adapter.query(
                      'ALTER TABLE ${widget.tableName} ALTER COLUMN "$newName" TYPE ${typeController.text}',
                    );
                  }

                  // Default
                  if (defaultController.text != (currentDefault ?? '')) {
                    if (defaultController.text.isEmpty) {
                      await adapter.query(
                        'ALTER TABLE ${widget.tableName} ALTER COLUMN "$newName" DROP DEFAULT',
                      );
                    } else {
                      await adapter.query(
                        'ALTER TABLE ${widget.tableName} ALTER COLUMN "$newName" SET DEFAULT ${defaultController.text}',
                      );
                    }
                  }

                  // Nullable
                  if (nullable != (isNullable == 'YES')) {
                    if (nullable) {
                      await adapter.query(
                        'ALTER TABLE ${widget.tableName} ALTER COLUMN "$newName" DROP NOT NULL',
                      );
                    } else {
                      await adapter.query(
                        'ALTER TABLE ${widget.tableName} ALTER COLUMN "$newName" SET NOT NULL',
                      );
                    }
                  }
